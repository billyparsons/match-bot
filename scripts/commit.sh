...
if echo "$MSG" | grep -qiE "feat:|new tool|new feature|add.*tool|tool.*add|behavior|default|workflow|breaking"; then
    /home/billy/cleo/venv/bin/python ~/cleo/scripts/notify_match.py "commit: \"$MSG\" — this looks like a user-facing change. read ~/.cleo/workspace/CODEBASE.md in full first, then make targeted edits only to what the commit message explicitly describes. do NOT infer behavior from file names, locations, or conventions — if the commit message does not clearly explain an architectural detail, text billy and ask before editing. text me when done or if nothing needed."
else
